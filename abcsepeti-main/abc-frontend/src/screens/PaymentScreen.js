  ...
  return (
    <>
      ...
      <Row className='mx-5 justify-content-md-center'>
        <h1 className='mx-5 justify-content-md-center'>{t('welcome:paymentScreen.payment.method')} </h1>
      </Row>
      <hr></hr>
      ...
      <Row>
        <Col xs={12} md={6}>
          {listLoading ? (
            ...
            <>
              <h2>{t('welcome:paymentScreen.select.payment.method')}</h2>
        ...
        <Col xs={12} md={6}>
          <h2>
            <p>{t('welcome:paymentScreen.addNewCard')}</p>
          </h2>
          <Row className='mx-5 justify-content-md-center'>
            <Col>
              <Form>
                <Form.Group controlId='cardNumber'>
                  <Form.Label>{t('welcome:paymentScreen.cardNumbar')}</Form.Label>
                  <Form.Control
                    type='text'
                    placeholder={t('welcome:paymentScreen.cardNumbar')}
                    value={cardNumber}
                    required
                ...
                <Form.Group controlId='expirationMonth'>
                  <Form.Label>{t('welcome:paymentScreen.expiration.month')}</Form.Label>
                  <Form.Control
                    type='text'
                    placeholder={t('welcome:paymentScreen.expiration.month')}
                ...
                <Form.Group controlId='expirationYear'>
                  <Form.Label>{t('welcome:paymentScreen.expiration.year')}</Form.Label>
                  <Form.Control
                    type='text'
                    placeholder={t('welcome:paymentScreen.expiration.year')}
                ...
                <Form.Group controlId='cvv'>
                  <Form.Label>{t('welcome:paymentScreen.cvv')}</Form.Label>
                  <Form.Control
                    type='password'
                    placeholder={t('welcome:paymentScreen.cvv')}
                    value={cvv}
                    required
          ...
          <Row className='mx-5 justify-content-md-center'>
            <Button type='submit' variant='primary' onClick={saveCard} disabled={saveLoading}>
              {saveLoading ? <Spinner as='span' animation='border' size='sm' role='status' aria-hidden='true' /> : <>{t('welcome:paymentScreen.addCard')}</>}
            </Button>
          </Row>
        </Col>
      </Row>
      ...
      <Row className='mx-5 justify-content-md-center'>
        <Button type='submit' variant='primary' onClick={proceedToPlaceOrder} className='mt-3' disabled={!paymentMethodId}>
          {t('welcome:paymentScreen.proceedToPlaceOrder')}
        </Button>
      </Row>
    </>
  );
