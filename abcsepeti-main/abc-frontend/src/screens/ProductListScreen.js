  ...
  return (
    <>
      <Row className='align-items-center'>
        <Col>
          <h1>{t('welcome:productList.products')}</h1>
        </Col>
        <Col className='text-right'>
          <Button className='my-3' onClick={createProductHandler}>
            <i className='fas fa-plus'></i> {t('welcome:productList.create.product')}
          </Button>
        </Col>
      </Row>
      ...
      {loading ? (
        ...
        <>
          <Table striped bordered hover responsive className='table-sm'>
            <thead>
              <tr>
                <th>{t('welcome:productList.id')}</th>
                <th>{t('welcome:productList.name')}</th>
                <th>{t('welcome:productList.price')}</th>
                <th>{t('welcome:productList.category')}</th>
                <th>{t('welcome:productList.imageId')} </th>
                <th>{t('welcome:productList.available.item.count')}  </th>
                <th>{t('welcome:productList.actions')}</th>
              </tr>
            </thead>
            <tbody>
