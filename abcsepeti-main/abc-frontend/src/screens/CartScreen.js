  ...
  return (
    <>
      {error ? (
        ...
        <>
          <Row>
            <h1>{t('welcome:cart.title.shopping.cart')}</h1>
          </Row>
          <Row>
            <Col md={8}>
              {cart == null || cart?.cartItems?.length == 0 ? (
                <Message>
                  {t('welcome:cart.your.cart.is.empty')} <Link to='/'>{t('welcome:cart.go.back')}</Link>
                </Message>
              ) : (
              ...
              <Row className='m-5 justify-content-md-center'>
                <LinkContainer to={'/'}>
                  <a>{t('welcome:cart.add.more.book')}</a>
                </LinkContainer>
              </Row>
            </Col>
            <Col md={4}>
                ...
                <ListGroup variant='flush'>
                  ...
                  <ListGroup.Item>
                    <Button type='button' className='btn-block' disabled={cart?.cartItems?.length === 0} onClick={checkoutHandler}>
                      {t('welcome:cart.proceed.to.checkout')}
                    </Button>
                  </ListGroup.Item>
                </ListGroup>
              </Card>
